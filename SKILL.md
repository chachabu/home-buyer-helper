...
- 交通情况（`--transport`，文字描述；`--nearest-metro` 邻近地铁站；`--metro-distance` 邻近地铁站步行距离，单位米）
...
- "给我推荐几套房源"
- "帮我筛选适合上班族租的近地铁房源"
- "总价500万以内，近地铁、户型好租的房源优先"
- "近地铁权重大一点，学区只做参考"
- "租售比2%以上只做参考，800米以内地铁优先"
...
3. 按匹配度排序推荐
   ...
   - 房龄 / 装修状况
   - 学区参考（低权重）
   - 租金 / 租售比只展示或做用户指定硬过滤，默认不参与评分
```

**推荐算法：**
- 预算/价格：只作为硬过滤，资金固定时不参与评分
- 租金 / 租售比：默认只展示和打标签，不参与评分；如用户明确要求，可通过 `--min-monthly-rent` / `--min-rent-yield` 做硬过滤，或手动设置 `--weight-rent`
- 默认 rental 权重：租金收益0、地铁通勤45、适租户型25、流动性15、房龄装修10、学区参考5
- 所有权重和阈值都必须作为执行参数支持覆盖，不能写死在使用流程里
- 默认参考标签：近地铁强推荐、地铁友好、上班族适租、高租金、高租售比、强租售比、学区参考
